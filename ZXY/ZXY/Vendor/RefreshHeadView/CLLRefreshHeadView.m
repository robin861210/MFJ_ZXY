...
{
    if (!_statusLabel) {
        _statusLabel = [[UILabel alloc] initWithFrame:CGRectMake(135,15, 100, 14)];
        _statusLabel.backgroundColor = [UIColor clearColor];
        _statusLabel.font = [UIFont systemFontOfSize:14.f];
//        _statusLabel.textColor = [UIColor blackColor];
        _statusLabel.textColor = UIColorFromHex(0x2b8eec);
    }
    return _statusLabel;
}
//时间
...
//        NSLog(@"CGRectGetMaxX(self.frame) : %f",CGRectGetMaxX(self.frame));
        _circleView = [[CLLRefreshCircleView alloc] initWithFrame:CGRectMake(80,15,CLLRefreshCircleViewHeight,CLLRefreshCircleViewHeight)];
    }
    return _circleView;
}
