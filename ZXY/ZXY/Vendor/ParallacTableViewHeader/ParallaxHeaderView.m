...
+ (id)parallaxHeaderViewWithCGSize:(CGSize)headerSize ImageArray:(NSArray *)imgStrArray
{
    ParallaxHeaderView *headerView = [[ParallaxHeaderView alloc] initWithFrame:CGRectMake(0, 0, headerSize.width, headerSize.height)];
    headerView.imgArray = [[NSMutableArray alloc] initWithArray:imgStrArray];
    [headerView initialSetup];
    return headerView;
}
...
- (void)initialSetup
{
    ...
    */
    ...
    for (int i = 0; i < self.imgArray.count; i++) {
        UIButton *imgButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width*i, 0, self.frame.size.width, self.frame.size.height)];
        [imgButton setBackgroundImageWithURL:[NSURL URLWithString:[self.imgArray objectAtIndex:i]] forState:UIControlStateNormal placeholderImage:LoadImage([self.imgArray objectAtIndex:i], @"png")];
        [self.imageScrollView addSubview:imgButton];
    }

    [self addSubview:self.imageScrollView];
    [self.imageScrollView setContentSize:CGSizeMake(self.frame.size.width * self.imgArray.count, self.frame.size.height)];
    
    UIImageView *leftImgView = [[UIImageView alloc] initWithFrame:CGRectMake(10, 70, 14, 17)];
    [leftImgView setBackgroundColor:[UIColor clearColor]];
    [leftImgView setImage:LoadImage(@"ADleft@2x", @"png")];
    [self addSubview:leftImgView];
    
    UIImageView *rightImgView = [[UIImageView alloc] initWithFrame:CGRectMake(self.frame.size.width-24, 70, 14, 17)];
    [rightImgView setBackgroundColor:[UIColor clearColor]];
    [rightImgView setImage:LoadImage(@"ADright@2x", @"png")];
