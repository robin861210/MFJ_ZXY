...
@synthesize myLocationCoordinate = _myLocationCoordinate;
@synthesize locationCoordinateFlag = _locationCoordinateFlag;
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    BOOL ret = [_mapManager start:@"zyFk1jmNLXhyA5GIGmBdyDqw"  generalDelegate:nil];
    if (!ret) {
        NSLog(@"manager start failed!");
    }
    [self getMapUserLocation];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(loginStateChange:)
                                               ...
                                               object:nil];
    [self MFJ_settingEaseMob:application Options:launchOptions];


    //判断是否登录 （用户IMLoginID和IMPassWD）
    if ([[[[UserInfoUtils sharedUserInfoUtils] infoDic] objectForKey:@"IMLoginID"] length] == 0 &&[[[[UserInfoUtils sharedUserInfoUtils] infoDic] objectForKey:@"IMPassWD"] length] == 0) {
...
- (void) getMapUserLocation
{
    ...
    self.locationCoordinateFlag = YES;
...
- (void)enterHomeViewController {
    ...
    if ([[[[UserInfoUtils sharedUserInfoUtils] infoDic] objectForKey:@"InvCode"] length] == 0) {
        NSString *EaseMobLoginStatusStr = [[[UserInfoUtils sharedUserInfoUtils] infoDic] objectForKey:@"EaseMobLoginStatus"];
        if ([EaseMobLoginStatusStr intValue])
        {
            //EaseMobLoginStatusStr = @"1"
            [self loginStateChange:nil];
        }else {
            //EaseMobLoginStatusStr = @"0"
            [self easeMobLogin];
        }
    }else
    {
...
- (void)didUpdateBMKUserLocation:(BMKUserLocation *)userLocation
{
    ...
    self.myLocationCoordinate = userLocation.location.coordinate;
    
    if (self.myLocationCoordinate.latitude != 0 && self.myLocationCoordinate.longitude != 0 && !isLocation) {
...
- (void)reverseGeocoding
{
    ...
    reverseGeoCodeSearchOption.reverseGeoPoint = self.myLocationCoordinate;
...
-(void) onGetReverseGeoCodeResult:(BMKGeoCodeSearch *)searcher result:(BMKReverseGeoCodeResult *)result errorCode:(BMKSearchErrorCode)error
{
    if (error == BMK_SEARCH_NO_ERROR) {
        ...
        if ([[[[UserInfoUtils sharedUserInfoUtils] infoDic] objectForKey:@"Lat"] length] == 0 &&[[[[UserInfoUtils sharedUserInfoUtils] infoDic] objectForKey:@"Lon"] length] == 0 && [[[[UserInfoUtils sharedUserInfoUtils] infoDic] objectForKey:@"LocaDesc"] length] == 0)
        {
            ...
            [addressInfoDic setValue:[NSString stringWithFormat:@"%f",self.myLocationCoordinate.latitude] forKey:@"Lat"];
            [addressInfoDic setValue:[NSString stringWithFormat:@"%f",self.myLocationCoordinate.longitude] forKey:@"Lon"];
            [addressInfoDic setValue:myAddress forKey:@"LocaDesc"];
